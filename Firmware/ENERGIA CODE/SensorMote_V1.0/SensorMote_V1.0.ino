...
void generateData(uint16_t* returnVal) { // this is were data is assinged to be sent via mote
  sensorData[0] = (int)(temp * 100);          // payload[1,2] = temp
  sensorData[1] = (int)(humidity * 100);      // payload[3,4] = humidity
  sensorData[2] = (int)(fLux * 100);          // payload[5,6] = light sensitivity
  sensorData[3] = (int)(oxygenData * 100);    // payload[7,8] = oxygen
  sensorData[4] = (int)(concentration * 100); // payload[9,10] = CO2 Concentration
  sensorData[5] = (int)(X_out * 100);         // payload[11,12] = Acceloromter C
  sensorData[6] = (int)(Y_out * 100);         // payload[13,14] = Acceloromter Y
  sensorData[7] = (int)(Z_out * 100);         // payload[15,16] = Acceloromter Z
  sensorData[8] = (int)(WindSpeed_MPH * 100); // payload[17,18] = Windspeed
  sensorData[9] = rainValue;                  // payload[19,20] =
